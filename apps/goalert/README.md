...
The database archives WAL continuously and takes a nightly base backup (04:00 UTC) to `s3://devopscoop-project1-dev-goalert-db-backups/` through the CNPG Barman Cloud Plugin (`apps/cnpg-barman-plugin`). The `ObjectStore` in `db-cluster.yaml` says where and for how long (30-day recovery window — barman deletes obsolete objects itself); the `ScheduledBackup` there says when. The bucket and the IRSA role the sidecars assume come from aws-eks-template (`cluster/cnpg-databases.tf` — one map entry per CNPG database, with an optional cross-region replication toggle for prod). Because WAL is archived too, this is point-in-time recovery to any moment inside the window, not just nightly snapshots. On EKS the database volumes use the `gp3-dangerous` StorageClass, which opts them out of the cluster-wide DLM EBS snapshots (barman makes those redundant) — see `apps/eks-storage-classes/gp3-dangerous.yaml` and the `dlm_snapshot_cnpg_databases` flag in aws-eks-template.
