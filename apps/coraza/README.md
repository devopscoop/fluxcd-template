...
- **Request** body inspection is broken in coraza-proxy-wasm 0.6.0 for bodies larger than one Envoy read chunk (~16KiB): the filter buffers the body for inspection but forwards it truncated, so the backend sees the body cut off mid-stream and returns its own 4xx (Grafana answered 400 "unexpected EOF" on every dashboard query, which the UI renders as empty panels). Bodies over `SecRequestBodyLimit` (131072, from `@recommended-conf`) get a clean 413 instead — and over HTTP/2 the 413 already fires from ~64KiB, the stream flow-control window. Both Gateways carry a `ClientTrafficPolicy` with `connection.bufferLimit: 256Ki` (`apps/eg-custom-resources/clienttrafficpolicy-{public,private}.yaml`), a prerequisite for buffering bodies up to the CRS limit that turned some silent truncations into clean 413s — but it does NOT stop the ~16KiB truncation; that's the wasm plugin, not the buffer. Until an upstream release fixes multi-chunk body handling, any host that must accept >16KiB request bodies needs a `per_authority_directives` entry pointing at the `no-request-body` directive set (grafana.project1-dev.devops.coop has one; note `api.project1-dev.devops.coop`/`app.project1-dev.devops.coop` POSTs over ~16KiB are still exposed to truncation). When bumping the wasm image, re-test with three POSTs through a gateway host — ~15KiB (expect 200), ~32KiB (200 once fixed; 400 while broken), ~200KiB (413) — before removing the exemptions.
- The CRS method allowlist is extended to `GET HEAD POST OPTIONS PUT PATCH DELETE` via `SecAction` id `900200` (`tx.allowed_methods`). The CRS default omits `PUT PATCH DELETE`, so rule `911100` returned a 403 for every REST mutation through the gateways. The override sits after `@crs-setup-conf` and before the `@owasp_crs` include because initialization rule `901160` only applies the default when `tx.allowed_methods` is still unset.
- CRS rule `920420` ("Request content type is not allowed by policy") is dropped for `/socket.io/` paths via a `ctl:ruleRemoveById` exclusion. socket.io long-polling POSTs use `Content-Type: text/plain;charset=UTF-8`, which the CRS default content-type allowlist rejects, and the socket.io client always connects over polling before upgrading to websocket — so the rule 403'd the connect and uptime-kuma's dashboard showed "Lost connection to the socket server". Request-body rules still apply on those paths.
