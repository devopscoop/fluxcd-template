...
- **values.yaml** keeps only the default route receiver (`route.receiver: default`) and `route.group_by` in its `>>> slack` marker block (uncommented by deploy.sh when `slack_alerts=true` in variables.sh).
