...
To enable it on an already-deployed cluster instead, uncomment the `>>> slack` block in values.yaml — run `./uncomment_blocks.sh slack` from the repo root, or by hand: strip the leading `#` and space between the markers, leaving the markers in place — and edit the webhook URLs into the encrypted secrets with `sops helm_secrets.yaml`.
