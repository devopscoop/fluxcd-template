...
- The chart's sync-job fetches the default dashboards and rules from raw.githubusercontent.com at install time, and Grafana downloads the VictoriaMetrics/VictoriaLogs datasource plugins plus the `grafana.dashboards` entries in values.yaml (currently the OpenCost dashboard) from grafana.com at startup — all of it needs internet egress.
