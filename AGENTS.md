...
- `./psql-oauth.sh APP -- -c 'SQL'` runs SQL on a CNPG database with the `pg-oauth` block enabled, as the developer's own OAuth role (`runbooks/connect-cnpg-database.md` option 4), and works without a terminal: it port-forwards, starts psql's device flow, opens dex's approval page in the developer's browser, and returns psql's output once they approve — so allow a few minutes for the run, and batch statements into one invocation (SQL on stdin works too), since every invocation is a fresh login. `-At` gives machine-readable output. The role defaults to the git email's local part; when the script says it isn't a login role, pass `-U ROLE` from the list it prints.
