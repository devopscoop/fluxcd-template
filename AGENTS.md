...
CI runs one job on pull requests: `.github/workflows/flux-diff.yaml` renders the cluster at the PR's merge-base and at its head offline with [flate](https://github.com/home-operations/flate) and posts the rendered diff as a sticky PR comment (README.md → "Continuous integration"). Two things about it are non-obvious. flate loads every YAML file in the directory it is pointed at and ignores kustomize `resources` lists, so the job generates a root Kustomization in `flux-diff-entry/` (gitignored) whose `spec.path` is `flux/flux-system` — the enabled set is then honored, the same way the FluxInstance's sync does it in the cluster. And because this template ships `resources: []`, the job falls into "template mode" on this repo: it enables every Kustomization whose `spec.path` exists, copies the tracked `*.decrypted` boilerplate to the real filenames, and renders with `--skip-schema-validation` (placeholders like falcon-platform's `CHANGEME-CID` fail chart schemas); cluster repos keep the schema check. A render failure fails the job, a diff does not — so a PR that breaks a kustomization or fails a chart's values schema goes red here before Flux sees it.
