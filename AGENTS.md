...
Optional config ships commented out between `>>> <marker>` / `<<< <marker>` comment delimiters (with a leading hash on the real markers). `uncomment_blocks.sh` strips the leading hash-and-space (or a lone hash) from the lines between them, leaving the markers in place so it stays idempotent. It only transforms files and prints what it touched — no git operations — so `deploy.sh` stages what it prints (the same split as `encrypt_secrets.sh`: the tool owns the mechanism, `deploy.sh` owns the git bookkeeping). Genuine comments inside a block must use a doubled hash (`## like this`); uncommenting leaves those untouched. The uncommenting steps run on every `deploy.sh` invocation, not just bootstrap: an app added later ships with its blocks still commented, and the next run opens (and commits) them. A block can also be opened without a full deploy by running `./uncomment_blocks.sh <marker>` directly. Current markers: `eks` (enabled when `k8s_platform=eks` — IRSA service-account annotations, AWS NLB annotations) and `slack` (enabled when `slack_alerts=true` — Alertmanager → Slack, in both `apps/kube-prometheus-stack` and `apps/victoria-metrics`; see their READMEs).

Gotcha, called out in `uncomment_blocks.sh` itself: never write the literal opening marker (hash, space, three `>`) anywhere except a real marker. The script greps every tracked file for it and rewrites every file that matches — including prose. When writing about markers in docs, drop the leading hash, as above.
