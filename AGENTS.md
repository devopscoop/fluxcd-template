...
Placement: stack-agnostic apps keep their scrape/rule CRs in `apps/victoria-metrics-custom-resources`, one `<app>-vm.yaml` file per app holding all of its scrape/rule objects (CRD-ordering and either/or-stack reasons — that kustomization's header explains); apps that only exist as part of the VictoriaMetrics stack (tempo, otel-collector, victoria-logs, goalert, uptime-kuma) carry theirs in-app with `dependsOn: victoria-metrics` on their flux Kustomization. Downloaded dashboards all live in `apps/victoria-metrics/values.yaml` whichever group the app belongs to — the Grafana that downloads them lives there (likewise the Alloy ones in `apps/kube-prometheus-stack/values.yaml` for the alternative stack); hand-built ConfigMap dashboards sit wherever fits.
