...
Optional config ships commented out between `>>> <marker>` / `<<< <marker>` comment delimiters (with a leading hash on the real markers). `toggle_blocks.sh --enable <marker>` strips the leading hash-and-space (or a lone hash) from the lines between them; `--disable <marker>` puts it back. The markers stay in place, so both directions are idempotent. It only transforms files and prints what it touched — no git operations — so `deploy.sh` stages what it prints (the same split as `encrypt_secrets.sh`: the tool owns the mechanism, `deploy.sh` owns the git bookkeeping). Genuine comments inside a block must use a doubled hash (`## like this`); toggling leaves those untouched in both directions. Files under `apps/templates/` are skipped (same as `encrypt_secrets.sh`) — the boilerplate always ships with its blocks commented out. The enabling steps run on every `deploy.sh` invocation, not just bootstrap: an app added later ships with its blocks still commented, and the next run opens (and commits) them. `deploy.sh` never disables — turning a feature back off is a manual run. A block can also be toggled without a full deploy by running `./toggle_blocks.sh --enable <marker>` (or `--disable`) directly; markers can be comma-separated or repeated, and modes mixed (`./toggle_blocks.sh --enable eks --disable slack`). Current markers: `eks` (enabled when `k8s_platform=eks` — EKS-platform config: IRSA service-account annotations, AWS NLB annotations, gp3 storage classes, disabling control-plane scrapes, plus the aws-load-balancer-controller/metrics-server monitoring entries in the victoria-metrics apps), `karpenter` (also enabled when `k8s_platform=eks` — pins spot-intolerant workloads to on-demand nodes via `karpenter.sh/capacity-type`, and gates Karpenter's own monitoring entries in the victoria-metrics apps; a separate marker because those labels exist only on Karpenter-provisioned nodes, and the pinning — or the monitoring, if Karpenter is removed — can be toggled independently of the EKS config), `opencost` (also enabled when `k8s_platform=eks` — OpenCost's monitoring entries in the victoria-metrics apps (scrape CR and dashboard download), gated with the app like metallb's), `slack` (enabled when `slack_alerts=true` — Alertmanager → Slack in `apps/victoria-metrics`; see its README), `dex` (never auto-enabled — toggled manually once the optional `apps/dex` issuer is deployed; in `apps/victoria-metrics` it turns on Grafana SSO via `auth.generic_oauth` plus a gateway-level OIDC `SecurityPolicy` for the Alertmanager/vmui routes; see `apps/dex/README.md`), `pg-oauth` (never auto-enabled — PostgreSQL 18 native OAuth for developer logins in the CNPG database template, validated against the dex issuer; separate from `dex` because it needs a hand-built validator extension image and Kubernetes 1.35+ image volumes first — enabling it with the placeholder image reference breaks the database pods; see the block's comments and `apps/dex/README.md`), `metallb` (enabled when `k8s_platform` is not eks — MetalLB's monitoring entries in the victoria-metrics apps (scrape/rule CRs and the dashboard download), gated with the app), and the per-app monitoring markers `longhorn`, `rook-ceph`, `rabbitmq`, `coraza`, `valkey`, and `uptime-kuma` (never enabled by `deploy.sh` — run `./toggle_blocks.sh --enable <app>` by hand when you enable that opt-in app, so its scrapes/rules/dashboards turn on with it; see the Monitoring section), plus `rabbitmq-instances` (inside `apps/victoria-metrics-custom-resources/rabbitmq-vm.yaml` — the broker-fleet scrape, enabled by hand when the first RabbitmqCluster ships; see that file's header) and `valkey-instances` (the same pattern inside `valkey-vm.yaml`, enabled by hand when the first ValkeyCluster ships).
...
- **Alert rules** are `VMRule` objects, adapted from the best upstream source with every deviation documented in the file header (the VMRule in `envoy-vm.yaml` is the style reference). Severity is a page label, not a filter — every warning and critical pages GoAlert and Slack — so upstream rule sets get pruned and re-tuned, not copied.
- **Dashboards** — prefer a chart-native toggle (`apps/cnpg`, which the sidecar picks up as a `grafana_dashboard: "1"` ConfigMap — it reads every namespace); otherwise add an entry under `grafana.dashboards` in `apps/victoria-metrics/values.yaml`, which Grafana's download-dashboards init container fetches at every pod start (`gnetId` for grafana.com dashboards, a pinned raw GitHub URL otherwise), with a comment naming what feeds it and any known gaps. Write a ConfigMap dashboard only when there is no upstream to pull (the hand-built envoy/coraza dashboards in `apps/victoria-metrics`, `apps/tempo/dashboard.yaml`).

Placement: apps outside the monitoring stack keep their scrape/rule CRs in `apps/victoria-metrics-custom-resources`, one `<app>-vm.yaml` file per app holding all of its scrape/rule objects (CRD-ordering reasons — that kustomization's header explains); apps that only exist as part of the VictoriaMetrics stack (tempo, otel-collector, victoria-logs, goalert, uptime-kuma) carry theirs in-app with `dependsOn: victoria-metrics` on their flux Kustomization. Downloaded dashboards all live in `apps/victoria-metrics/values.yaml` whichever group the app belongs to — the Grafana that downloads them lives there; hand-built ConfigMap dashboards sit wherever fits.

Gating: monitoring for an app that isn't deployed must not apply — an empty scrape pool pages `ScrapePoolHasNoTargets`. Entries for platform-conditional and opt-in apps therefore sit in marker blocks — scrape/rule CRs in the kustomization `resources` lists, dashboard downloads under `grafana.dashboards` in `apps/victoria-metrics/values.yaml` — enabled with the app (`eks`/`karpenter`/`opencost` and `metallb` automatically by deploy.sh; `longhorn`/`rook-ceph`/`rabbitmq`/`coraza` by hand). deploy.sh additionally *deletes* `apps/victoria-metrics-custom-resources/<app>-vm.yaml` for every app the cluster doesn't run (its removal step; converges on every run) — so enabling an opt-in app on a bootstrapped cluster means restoring its file from fluxcd-template before toggling the marker. When adding an app, add its three pillars the same way — and if it's opt-in, a marker named after it.
...
CI runs one job on pull requests: `.github/workflows/flux-diff.yaml` runs `./flux_diff.sh` (see "Other scripts") and posts the rendered diff as one sticky PR comment through `.github/scripts/flux-diff-comment.js` (README.md → "Continuous integration"). The script's exit code drives the job: 1 (this tree fails to render) fails it; 0 and 2 (only the merge-base fails) pass. Two things about the script are non-obvious and spelled out in its header: flate loads every YAML file in the directory it is pointed at and ignores kustomize `resources` lists, so the script renders through a generated root Kustomization whose `spec.path` is `flux/flux-system`; and because this template ships `resources: []`, the script falls into "template mode" here — every Kustomization whose `spec.path` exists, the tracked `*.decrypted` boilerplate copied to the real filenames, chart schema validation off. So a PR that breaks a kustomization or fails a chart's values schema goes red in CI before Flux sees it, and `./flux_diff.sh` shows the same thing locally.

## Other scripts

- `./flux_diff.sh [-o STYLE] [BASE_REV]` renders the working tree (uncommitted changes included) and the merge-base with `BASE_REV` (default `origin/main`) offline with flate and prints what Flux would apply differently. It renders from copies, so the working tree is untouched. Exit 1 means this tree fails to render, 2 means only the base does. Needs `flate` (Brewfile cask; no Arch package, see pkglist.txt).
...
- `./psql-oauth.sh APP -- -c 'SQL'` runs SQL on a CNPG database with the `pg-oauth` block enabled, as the developer's own OAuth role (`runbooks/connect-cnpg-database.md` option 4), and works without a terminal: it port-forwards, starts psql's device flow, opens dex's approval page in the developer's browser, and returns psql's output once they approve — so allow a few minutes for the run. Every fresh connection is a new approval, so for more than one or two queries start a session first: `./psql-oauth.sh --session start APP` (one approval, blocks until the developer approves), after which the same `APP -- -c 'SQL'` runs (also `-f FILE`, SQL on stdin, and the output flags `-A -t -x --csv`) are answered through the background psql with no browser, exit 1 on a server error, and `./psql-oauth.sh --session stop APP` ends it when the work is done. `-At` gives machine-readable output. The role defaults to the git email's local part; when the script says it isn't a login role, pass `-U ROLE` from the list it prints.
